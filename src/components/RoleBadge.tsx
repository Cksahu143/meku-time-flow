...
const ROLE_GRADIENTS: Record<AppRole, string> = {
  ...
  school_admin: 'from-orange-500 to-amber-500',
  platform_admin: 'from-purple-500 to-pink-500',
};

export function RoleBadge({ role, showIcon = true, size = 'sm', className, animated = false }: RoleBadgeProps) {
  ...
  const badge = (
    <Badge
      variant="secondary"
      className={cn(
        'font-semibold gap-1.5 border-0 text-white shadow-sm',
...
export function RoleCard({ className }: { className?: string }) {
  const { userRole, permissions, schoolId, loading } = useRBAC();
    ...
    >
      ...
      <div className="relative z-10 flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-white/80 mt-0.5">{roleDescriptions[userRole]}</p>
