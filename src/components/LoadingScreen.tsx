  ...
  return (
    <motion.div
      ...
      style={{
        width: size,
        height: size,
        background: index % 2 === 0 ? 'hsl(var(--primary))' : 'hsl(var(--accent))',
        top: '50%',
        left: '50%',
      }}
      animate={{ rotate: [angle, angle + 360] }}
      transition={{ duration, repeat: Infinity, ease: 'linear', delay: index * 0.15 }}
    >
      <motion.div
        style={{
          position: 'absolute',
...
const FloatingParticle = ({ index }: { index: number }) => {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: [0, 0.8, 0], scale: [0, 1, 0], y: [0, -30 - Math.random() * 40, -60] }}
      transition={{ duration: dur, repeat: Infinity, delay: index * 0.4, ease: 'easeInOut' }}
    />
  );
};
...
export function LoadingScreen() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background overflow-hidden">
      ...
      <GlowOrb color="var(--primary)" size={350} x="25%" y="30%" delay={0} blur={100} />
      <GlowOrb color="var(--accent)" size={280} x="70%" y="65%" delay={1.5} blur={80} />
      <GlowOrb color="var(--success)" size={200} x="50%" y="50%" delay={3} blur={60} />
      ...
      <AnimatePresence>
        {showContent && (
          ...
          >
            {/* Logo with orbiting particles + animated gradient border */}
            <motion.div
            ...
            >
              ...
              <motion.div
                className="absolute inset-0 rounded-3xl bg-gradient-primary shadow-primary flex items-center justify-center overflow-hidden"
                animate={{ rotate: [0, 2, -2, 0] }}
                transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}
              >
                <svg viewBox="0 0 40 40" className="w-14 h-14 text-primary-foreground relative z-10" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                  <motion.path d="M8 10 L8 34 L32 34 L32 10 Z" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 1.2, delay: 0.5, ease: 'easeInOut' }} />
                  <motion.path d="M16 10 L16 34" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.6, delay: 0.9 }} />
                  <motion.path d="M20 18 L28 18" stroke="hsl(var(--accent))" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.4, delay: 1.1 }} />
                  <motion.path d="M20 23 L28 23" stroke="hsl(var(--accent))" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.4, delay: 1.3 }} />
                  <motion.path d="M20 28 L26 28" stroke="hsl(var(--accent))" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 0.3, delay: 1.5 }} />
                </svg>
                {/* Holographic shimmer over logo */}
                <motion.div
                  className="absolute inset-0"
            ...
            {/* App Name */}
            <motion.div className="space-y-1" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}>
              <motion.h1
            ...
            {/* Progress Bar */}
            <motion.div className="w-64 mx-auto space-y-3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }}>
